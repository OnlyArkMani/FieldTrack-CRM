// Validates the offline-vs-real-error split on the Follow-ups screen: a
...
import 'package:fieldtrack_mobile/features/crm/followups/data/follow_up_repository.dart';
import 'package:fieldtrack_mobile/features/crm/followups/screens/followups_screen.dart';

Future<void> _pump(WidgetTester tester, Object error) async {
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        myFollowUpsProvider.overrideWith(
...
void main() {
  testWidgets(
    ...
    expect(find.text('No follow-ups saved offline yet'), findsOneWidget);
  ...
  testWidgets('still shows the real error page + Retry for a genuine error',
    ...
    expect(find.text('No follow-ups saved offline yet'), findsNothing);
  });
}
