...
- No auto _check-in_ exists anywhere in the backend or mobile app.
...
1. **Item 1:** Confirm there's no auto _check-in_ anywhere to remove (only auto clock-out-on-logout was found) — or point to where auto check-in happens if it exists elsewhere.
