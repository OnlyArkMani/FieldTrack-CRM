 ...
 *   simulating 2 managers polling the live team view and occasionally
...
export const options = {
  scenarios: {
    ...
    admin_dashboard: {
      executor: "constant-vus",
      vus: Math.min(2, managers.length || 1),
  ...
  if (Math.random() < 0.12) {
    ...
    if (session.stage === "NONE" || session.stage === "ENDED") {
      ...
      res = http.post(`${API}/attendance/break`, JSON.stringify({ lat, lng }), {
        ...headers,
        tags: { name: "attendance_break" },
      });
      label = "break";
      if (res.status === 200) session.stage = "ON_BREAK";
    } else if (session.stage === "ON_BREAK") {
      res = http.post(
        `${API}/attendance/resume`,
        JSON.stringify({ lat, lng }),
        {
          ...headers,
          tags: { name: "attendance_resume" },
        },
      );
      label = "resume";
      if (res.status === 200) session.stage = "STARTED";
      ...
      res = http.post(
        ...
        JSON.stringify({
          lat,
          lng,
          work_summary: "Load test synthetic shift.",
        }),
    ...
    const ok = check(res, {
      [`attendance ${label}: 2xx`]: (r) => r.status >= 200 && r.status < 300,
    });
...
export function managerFlow() {
  const session = getSession(managers.length ? managers : employees);
  if (!session.token) {
    sleep(2);
    return;
  }
  ...
  const res = http.get(`${API}/location/team-live`, {
    ...headers,
    tags: { name: "team_live" },
  });
